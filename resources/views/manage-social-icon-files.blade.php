...
<div class="container-fluid">
    ...
    @error('images')
       <div class="alert alert-warning">
            Please choose a category and upload a file
        </div>
    @enderror
    <div class=" card my-4 p-5">
        <form action="{{route('social.icon.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-4">
                    <h4 class=" mb-1 text-dark">Please select a catagory </h4>
                </div>
            </div>
            <div class="row">
                <div class="col-4">
                    <select name="category" class="custom-select text-dark mb-3" id="cotegory">
                        <option class="bg-white text-dark" disabled selected>Select Category</option>
                        @foreach ($categories as $category)
                            <option class="bg-white text-dark" value="{{$category->social_icon}}">{{$category->social_icon}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-1">
            ...
            <div class="row" id="socialIconUpload"></div>
