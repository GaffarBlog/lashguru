<!-- Begin Cat Search Filters Page Content -->
@extends('main')
@section('title') {{'Search Filters'}} @endsection
@section('main-container')
<!-- Cat Search Filters Page Start -->
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Admin Page: Search Filters</h1>
            ...
            <div class="card p-5 mt-5 mb-5">
                ...
                @if ( isset($_GET['leveltwo']) && isset($_GET['levelthree']) && isset($_GET['levelfour']) )
                    <div class="d-flex justify-content-between">
                        <h4 class="text-dark">Search Filters: {{ $level_four->count() }} Categories</h4>
                        <div class="d-flex justify-content-between">
                ...
                @elseif ( isset($_GET['leveltwo']) && isset($_GET['levelthree']) )
                    <div class="d-flex justify-content-between">
                        <h4 class="text-dark">Search Filters: {{ $level_three->count() }} Categories</h4>
                        <div class="d-flex justify-content-between">
                ...
                @elseif( isset($_GET['leveltwo']) )
                    <div class="d-flex justify-content-between">
                        <h4 class="text-dark">Search Filters: {{ $level_two->count() }} Categories</h4>
                        <div class="d-flex justify-content-between">
                            <a href="cat-search-filters" rel="noopener noreferrer"><i class="fas fa-angle-left"> Level One</i></a>
                        </div>
                    </div>
                @else
                    <h4 class="text-dark">Search Filters: {{ $cat_search_filters->count() }} Categories</h4>
                @endif
                ...
                <table class="table table-striped mt-4">
                    ...
                    <tbody id="cwBusinessCat">
                            ...
                            @if (!empty($level_three) && count($level_three))
                                @foreach($level_three as $level)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <a href="?leveltwo={{$level->level_two }}&levelthree={{ $level->level_three }}&levelfour={{ $level->id }}">{{ $level->filter_name }}</a>
                        ...
                        @elseif( isset($_GET['leveltwo']) )
                            @if (!empty($level_two) && count($level_two))
                                @foreach($level_two as $level)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <a href="?leveltwo={{ $level->level_two }}&levelthree={{ $level->id }}">{{ $level->filter_name }}</a>
                                                <span class="badge badge-pill badge-secondary">{{ $count[$index] }}</span>
                                            </div>
                                        </td>
                                        <td class="align-middle">
                            ...
                            @endif
                        @else
                            @if( !empty($cat_search_filters) && count($cat_search_filters) )
                                @foreach($cat_search_filters as $category)
                                    <tr>
                                        <td class="align-middle">{{ $category->id }}</td>
                                        <td class="align-middle">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <a href="?leveltwo={{ $category->id }}">{{ $category->filter_name }}</a>
                                                <span class="badge badge-pill badge-secondary">{{ $count[$index] }}</span>
                                            </div>
                                        </td>
                                        <td class="align-middle">
...
@endsection
<!-- End Cat Search Filters -->
<!-- Add Script File -->
@push('js')
    <script src="{{ asset('js/cat-search-filters.js?' . time() ) }}"></script>
@endpush
<!-- End Script File -->