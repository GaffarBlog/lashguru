<!-- Begin Cat Manage SVG Categories Page Content -->
@extends('main')
@section('title') {{'SVG Files Categories'}} @endsection
@section('main-container')
<!-- Cat Manage SVG Categories Page Start -->
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Admin Page: Manage SVG Categories</h1>
            ...
            <div class="card p-5 mt-5 mb-5">
                ...
                @if ( isset($_GET['leveltwo']) && isset($_GET['levelthree']) )
                    <div class="d-flex justify-content-between">
                        <h4 class="text-dark">Manage SVG Categories: {{ $level_three->count() }} Categories</h4>
                        <div class="d-flex justify-content-between">
                ...
                @elseif( isset($_GET['leveltwo']) )
                    <div class="d-flex justify-content-between">
                        <h4 class="text-dark">Manage SVG Categories: {{ $level_two->count() }} Categories</h4>
                        <div class="d-flex justify-content-between">
                            <a href="cat-manage-svg-files" rel="noopener noreferrer"><i class="fas fa-angle-left"> Level One</i></a>
                        </div>
                    </div>
                @else
                    <h4 class="text-dark">Manage SVG Categories: {{ $cat_manage_svg_files->count() }} Categories</h4>
                @endif
                ...
                <table class="table table-striped mt-4">
                    ...
                    <tbody id="cwBusinessCat">
                        ...
                        @elseif( isset($_GET['leveltwo']) )
                            @if (!empty($level_two) && count($level_two))
                                @foreach($level_two as $level)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <a href="?leveltwo={{ $level->level_two }}&levelthree={{ $level->id }}">{{ $level->svg_file }}</a>
                            ...
                            @endif
                        @else
                            @if( !empty($cat_manage_svg_files) && count($cat_manage_svg_files) )
                                @foreach($cat_manage_svg_files as $category)
                                    <tr>
                                        <td class="align-middle">{{ $category->id }}</td>
                                        <td class="align-middle">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <a href="?leveltwo={{ $category->id }}">{{ $category->svg_file }}</a>
                                                <span class="badge badge-pill badge-secondary">{{ $count[$index] }}</span>
                                            </div>
                                        </td>
                                        <td class="align-middle">
...
@endsection
<!-- End Cat Manage SVG Categories -->
<!-- Add Script File -->
@push('js')
    <script src="{{ asset('js/cat-manage-svg-files.js?' . time() ) }}"></script>
@endpush
<!-- End Script File -->
