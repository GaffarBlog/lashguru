$(document).ready(function() {
    ...
    $(cwEditCategory).on('click', function() {
        ...
        $.ajax({
            type: "GET",
            url: "/cat-manage-tasks/edit/" + id,
            data: {
                '_token' : token,
                'id' : id
            },
            success: function (response) {
                let businessCat = response.cat_manage_tasks;
                if( businessCat ) {
                    $('#cwUpdateCatName').val(businessCat.task_name);
                }
            }
        });
    });
    ...
    $('.cw-update-category').on('click', function() {
        ...
        $.ajax({
            type : 'PUT',
            url : '/cat-manage-tasks/update/' + id,
            dataType: 'json',
            data : {
                '_method': 'PUT',
    ...
    $('.cw-delete-category').on('click', function() {
        ...
        $.ajax({
            type : 'GET',
            url : '/cat-manage-tasks/destroy/' + id,
            dataType: 'json',
            data : {
                '_token' : token,
                'id' : id,
