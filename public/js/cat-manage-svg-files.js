$(document).ready(function() {
    ...
    $(cwEditCategory).on('click', function() {
        ...
        $.ajax({
            type: "GET",
            url: "/cat-manage-svg-files/edit/" + id,
            data: {
                '_token' : token,
                'id' : id
            },
            success: function (response) {
                let businessCat = response.cat_manage_svg_files;
                if( businessCat ) {
                    $('#cwUpdateCatName').val(businessCat.svg_file);
                }
            }
        });
    });
    ...
    $('.cw-update-category').on('click', function() {
        ...
        $.ajax({
            type : 'PUT',
            url : '/cat-manage-svg-files/update/' + id,
            dataType: 'json',
            data : {
                '_method': 'PUT',
    ...
    $('.cw-delete-category').on('click', function() {
        ...
        $.ajax({
            type : 'GET',
            url : '/cat-manage-svg-files/destroy/' + id,
            dataType: 'json',
            data : {
                '_token' : token,
                'id' : id,
