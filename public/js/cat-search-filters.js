$(document).ready(function() {
    ...
    $(cwEditCategory).on('click', function() {
        ...
        $.ajax({
            type: "GET",
            url: "/cat-search-filters/edit/" + id,
            data: {
                '_token' : token,
                'id' : id
            },
            success: function (response) {
                let businessCat = response.cat_search_filters;
                if( businessCat ) {
                    $('#cwUpdateCatName').val(businessCat.filter_name);
                }
            }
        });
    });
    ...
    $('.cw-update-category').on('click', function() {
        ...
        $.ajax({
            type : 'PUT',
            url : '/cat-search-filters/update/' + id,
            dataType: 'json',
            data : {
                '_method': 'PUT',
    ...
    $('.cw-delete-category').on('click', function() {
        ...
        $.ajax({
            type : 'GET',
            url : '/cat-search-filters/destroy/' + id,
            dataType: 'json',
            data : {
                '_token' : token,
                'id' : id,
